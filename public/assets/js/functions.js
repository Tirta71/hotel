...
jQuery(function () {
  ...
  if (!navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) {
    jQuery(window)
      .on("touchstart.leluxe", themeTouchStart)
      .on("mousemove.leluxe", themeMouseMove);
  ...
  if (jQuery("html").hasClass("touchevents")) {
    ...
    ).on("focus.leluxe blur.leluxe", function (e) {
      if (
        screen_has_mouse &&
  ...
  jQuery(window).on("hashchange", function () {
    ...
    var $active_tab_content = jQuery(
      '.tab-content[data-id="' + window.location.hash.substring(1) + '"]'
    );
    ...
    $tab_container
      .find(".tab-content:not(#" + $active_tab_content.data("id") + ")")
  ...
  jQuery(document).on("click.leluxe", function (e) {
    if (
      jQuery(e.target).parents(".share-widget-container").length > 0 ||
      jQuery(e.target).hasClass("share-widget-container")
    ) {
      return;
    }
    jQuery(".share-widget-container.show-dropdown").removeClass(
      "show-dropdown"
    );
  });
  ...
  jQuery(document).on("click.leluxe", function (e) {
    if (
      jQuery(e.target).parents(".Zebra_DatePicker").length > 0 ||
      jQuery(e.target).hasClass("Zebra_DatePicker") ||
      ((jQuery(e.target).parent("a.mdi").length > 0 ||
        jQuery(e.target).is("a.mdi")) &&
        jQuery(e.target).parents(".booking-form .field").length > 0)
    ) {
      return;
    }
  ...
  if (jQuery.fn.Zebra_DatePicker) {
    ...
    var onSelectDate = function (format, date, dateObj) {
      ...
      jQuery(".short", $field).html(
        dateObj.getDate() +
          " " +
          jQuery.fn.Zebra_DatePicker.defaults.months[dateObj.getMonth()].slice(
            0,
            3
          ) +
          " " +
          (dateObj.getYear() + "").slice(-2)
      );
      jQuery(".long", $field).html(
        dateObj.getDate() +
          " " +
          jQuery.fn.Zebra_DatePicker.defaults.months[dateObj.getMonth()] +
          " " +
          dateObj.getFullYear()
      );
      if ($field.hasClass("field-checkin") && win_width >= 568) {
        jQuery(".booking-form .field-date.field-checkout").addClass(
          "show-dropdown"
        );
      }
    };
    ...
    jQuery(
      ".booking-form-full .field-checkin .custom-date-field"
    ).Zebra_DatePicker({
    ...
    jQuery(
      ".booking-form-full .field-checkout .custom-date-field"
    ).Zebra_DatePicker({
  ...
  if (jQuery.fn.owlCarousel) {
    ...
    var onTranslate = function (event) {
      ...
      onTranslated = function (event) {
        ...
        if (
          jQuery(".owl-item.active .light-hero-colors", event.target).length > 0
        ) {
    ...
    $hero_carousel.owlCarousel({
      ...
      onInitialized: function (event) {
        ...
        jQuery(
          '<div class="owl-expand"><a href="#"><span class="mdi"></span></a></div>'
        )
          .insertAfter(jQuery(".owl-nav", event.target))
          .on("click.leluxe", function (e) {
            e.preventDefault();
            ...
            if (!$body.hasClass("has-booking")) {
              var $nav_buttons = jQuery(this).add(
                jQuery(this).prev(".owl-nav")
              );
              $nav_buttons.animate(
                {
        ...
        jQuery(".owl-stage", event.target).on(
          "touchstart.leluxe",
          function (e) {
            if (!tapedTwice) {
              tapedTwice = true;
              ...
              $hero_carousel
                .find(".owl-expand:visible")
                .trigger("click.leluxe");
            }
          }
        );
    ...
    if ($body.hasClass("mobile-menu-opened")) {
      ...
      if ($body.hasClass("full-content")) {
        ...
        var contentTop = parseInt($content_wrap.css("padding-top"), 10),
          logoHeight =
            jQuery(".logo", $logo).outerHeight() + $logo.offset().top * 2;
